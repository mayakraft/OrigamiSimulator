<script>
  ...
  const saveFoldFile = () => {
    const FOLD = Settings.exportModel();
...
<div class={"container"}>
  ...
  <input
    ...
    value="euler"
    bind:group={Settings.integration} />
  ...
  <input type="range" min="10" max="100" step="1" bind:value={Settings.axialStiffness} />
  ...
  <input type="range" min="0" max="3" step="0.01" bind:value={Settings.creaseStiffness} />
  ...
  <button disabled={!Settings.active} on:click={Settings.reset}>reset model</button>
  <br />
