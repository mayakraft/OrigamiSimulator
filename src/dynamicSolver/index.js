...
const DynamicSolver = () => {
	...
	const updateMaterials = ({ meta, beamMeta }, initing = false) => {
		let index = 0;
		for (let i = 0; i < model.nodes.length; i += 1) {
			if (initing) {
				meta[4 * i] = index;
	...
	const updateExternalForces = ({ externalForces }) => {
		for (let i = 0; i < model.nodes.length; i += 1) {
	...
	const updateOriginalPosition = ({ originalPosition }) => {
		for (let i = 0; i < model.nodes.length; i += 1) {
	...
	const updateCreaseVectors = ({ creaseVectors }) => {
	...
	const updateCreasesMeta = ({ creaseMeta }, initing = false) => {
		for (let i = 0; i < model.creases.length; i += 1) {
	...
	const initTypedArrays = () => {
		...
		// these are now returned and delivered directly to the next method
		...
		updateOriginalPosition({ originalPosition });
		updateMaterials({ meta, beamMeta }, true);
		updateFixed({ mass });
		updateExternalForces({ externalForces });
		updateCreasesMeta({ creaseMeta }, true);
		updateCreaseVectors({ creaseVectors });
		return {
			position,
			lastLastPosition,
			velocity,
	 ...
	 */
	const solve = (numSteps = 100, props = {}) => {
	...
	const setModel = (newModel, options = {}) => {
		...
		model = newModel;
		const arrays = initTypedArrays();
		initTexturesAndPrograms(arrays, options);
		setSolveParams();
	};
	/**
