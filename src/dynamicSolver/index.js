...
class DynamicSolver {
	 ...
	 */
	nodeDidMove() {
		if (!this.gpuMath || !this.model) { return; }
	...
	solve(numSteps = 100, computeStrain = false) {
		if (!this.gpuMath || !this.model) { return 0; }
		...
		return render(
			this.gpuMath,
			this.model,
			{ textureDim: this.textureDim, axialStrain: computeStrain },
		);
	}
	...
	reset() {
		...
		return render(
			this.gpuMath,
			this.model,
			{ textureDim: this.textureDim, axialStrain: true },
		);
	}
	...
	setCreasePercent(value) {
		...
		this.gpuMath.setUniformForProgram("positionCalcVerlet", "u_creasePercent", number, "1f");
	}
	...
	setAxialStiffness(value) {
		...
		this.gpuMath.setProgram("positionCalcVerlet");
		this.gpuMath.setUniformForProgram("positionCalcVerlet", "u_axialStiffness", number, "1f");
	}

	setFaceStiffness(value) {
		if (!this.gpuMath || !this.model) { return; }
		const number = typeof value === "number" ? value : parseFloat(value);
		this.gpuMath.setProgram("velocityCalc");
		this.gpuMath.setUniformForProgram("velocityCalc", "u_faceStiffness", number, "1f");
		this.gpuMath.setProgram("positionCalcVerlet");
		this.gpuMath.setUniformForProgram("positionCalcVerlet", "u_faceStiffness", number, "1f");
	}
	...
	setFaceStrain(value) {
		...
		this.gpuMath.setProgram("positionCalcVerlet");
		this.gpuMath.setUniformForProgram("positionCalcVerlet", "u_calcFaceStrain", number, "1f");
	}
